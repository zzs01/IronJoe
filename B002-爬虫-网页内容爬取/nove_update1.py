...
def get_novel_link(novel_name, searchkey):
    home_page = "http://www.shuquge.com/"
#第二个源    https://www.shujy.com/ 章节名字不一样
    html = requests.get(home_page,headers=headers)
    
    home_page_link = home_page + "/search.php?searchkey=" + searchkey
    ...
    match_key = r'<h4 class="bookname"><a href="/(.*?)/index.html">%s</a></h4>'%searchkey
    # 小说不存在
    try:
